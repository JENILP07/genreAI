...
export const PredictionResults = ({ 
  ...
  return (
    ...
    >
      {/* Main Result Card */}
      <div className={`glass-card p-8 text-center relative overflow-hidden ${confidence > 70 ? 'ring-1 ring-accent/50 shadow-[0_0_30px_hsl(var(--accent)/0.2)]' : ''}`}>
        {/* Background glow */}
        <div 
        ...
        <div className="relative z-10">
          ...
          >
            <h2 className="text-3xl font-bold capitalize mb-2 text-primary transition-colors duration-300">
              {predictedGenre}
            </h2>
            <div className="flex items-center justify-center gap-2 text-muted-foreground mb-4">
              <CheckCircle2 className={`w-5 h-5 ${confidence > 80 ? 'text-primary' : 'text-muted-foreground'}`} />
      ...
      <div className="glass-card p-6">
        ...
        <div className="space-y-3">
          {allProbabilities.map((item, index) => {
            const isTop = item.genre === predictedGenre;
            return (
              <motion.div
                key={item.genre}
              ...
              >
                <div className="flex items-center justify-between text-sm">
                  <div className={`flex items-center gap-2 ${isTop ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>
                ...
                <div className="h-2 rounded-full bg-white/5 overflow-hidden">
                  <motion.div
                    ...
                    className={`h-full rounded-full transition-colors duration-500 ${isTop ? 'bg-primary' : 'bg-muted-foreground/30'}`}
                  />
                </div>
              </motion.div>
            );
        ...
        {top3.map((item, index) => {
          ...
          return (
            <motion.div
              ...
              className={`glass-card p-4 text-center ${isTop ? 'ring-2 ring-primary/50' : ''}`}
            >
              <div className="text-2xl mb-1">{GENRE_ICONS[item.genre]}</div>
              <div className={`text-sm font-semibold capitalize ${isTop ? 'text-primary' : 'text-muted-foreground'}`}>
                {item.genre}
              </div>
